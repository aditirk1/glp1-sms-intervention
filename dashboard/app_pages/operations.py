...
from common import TASK_LABELS, TIER_LABELS, api_get, api_post, tier_dot_html, titleise
...
with st.container(border=True):
    ...
    st.caption(
        "Posts through the live webhook so you can show score → rules → SMS "
            ...
            if not chosen_reply:
                ...
                if response is not None and response.status_code == 200:
                    ...
                    if result.get("error"):
                        ...
                        cols[2].metric(
                            "Barrier", titleise(result.get("barrier_type"))
                        )
                        cols[3].metric(
