...
with page_shell():
    ...
    status_filter = filters[1].selectbox(
        "Status",
        ["all", "active", "paused", "discontinued"],
        format_func=sentence_case,
    )
    sort_by = filters[2].selectbox(
        "Sort by",
        ["risk", "silence", "due", "weeks", "name"],
        format_func=sentence_case,
    )
    ...
    if tier_filter != "all":
        params["tier"] = {"high": "red", "medium": "amber", "low": "green"}[tier_filter]
    ...
    if not roster["items"]:
            ...
            [
                {
                    ...
                    "Status": sentence_case(p.get("status")),
                    "Risk": p.get("risk_score"),
                    "Barrier": sentence_case(p.get("barrier_type")) or None,
        ...
        def risk_style(value) -> str:
            if value is None or pd.isna(value):
                return ""
            # Match risk_tier_for_score: red above 0.60, amber through 0.60, else low.
            if value > RED_THRESHOLD:
                return "background-color: #fecaca; color: #991b1b;"
            if value >= AMBER_THRESHOLD:
                return "background-color: #fde68a; color: #92400e;"
