...
with page_shell():
    ...
    def _kind_label(kind: str) -> str:
        if kind == "all":
            return "All"
        return TASK_LABELS.get(kind, sentence_case(kind))
    ...
    queue_kind = filters[1].selectbox(
        "Type",
        kind_options,
    ...
    if not queue["items"]:
        ...
        for task in queue["items"]:
            ...
            with st.container(border=True):
                ...
                row[1].markdown(
                    f"**{TASK_LABELS.get(task['kind'], sentence_case(task['kind']))}**"
                )
                row[1].caption(
                    f"Priority {task['priority']} · {sentence_case(task['status'])}"
                )

                row[2].caption(
                    f"Week {task.get('weeks_on_therapy', '—')} · "
                    f"{sentence_case(task.get('barrier_type')) or 'No barrier'}\n\n"
